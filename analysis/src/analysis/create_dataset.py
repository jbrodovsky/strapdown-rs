...
Each configuration is run with both UKF and EKF filters for comparison.
Outputs are written to <output_root>/<filter_type>/<config_name>/<input_basename>.csv

Usage:
  python scripts/create_dataset.py [--bin /path/to/strapdown-sim] [--input data/input/] [--output-root data/] [--jobs N]
"""
...
def run_job(
    ...
    cmd = build_cmd(bin_path, input_path, out_path, cfg_args, filter_type)
...
def main(argv: List[str] | None = None) -> int:
    ...
    # prepare jobs: iterate over filter types, configs, and input files
    jobs = []
    for filter_type in FILTER_TYPES:
        for cfg_name, cfg_args in CONFIGS:
            for input_path in inputs:
                jobs.append((filter_type, cfg_name, input_path, cfg_args))
    ...
    if args.jobs <= 1:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.jobs) as ex:
            ...
            for fut in concurrent.futures.as_completed(future_to_job):
                filter_type, cfg_name, input_path = future_to_job[fut]
                try:
                    rc = fut.result()
                    if rc != 0:
                        print(
