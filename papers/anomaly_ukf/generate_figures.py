...
def create_system_architecture():
    ...
    for ax, title, use_geo in [(ax1, "Standard GNSS-Aided INS", False), (ax2, "Geophysically-Aided INS", True)]:
        ax.set_xlim(0, 10)
        ax.set_ylim(0, 10)
        ax.axis("off")
        ...
        # IMU input
        imu_box = FancyBboxPatch(
            (0.5, sensors_y),
            1.5,
            box_height,
            boxstyle="round,pad=0.1",
            facecolor=TEMPLE_LIGHT,
            edgecolor=TEMPLE_CHERRY,
            linewidth=2,
        )
        ax.add_patch(imu_box)
        ax.text(
            1.25,
            sensors_y + box_height / 2,
            "IMU\n(Accel/Gyro)",
            ha="center",
            va="center",
            fontsize=9,
        ...
        ax.text(
            1.25, sensors_y - 4 + box_height / 2, "Barometer", ha="center", va="center", fontsize=9, fontweight="bold"
        )
        ...
        ax.text(output_x + 0.75, 5.25, "Navigation\nSolution", ha="center", va="center", fontsize=9, fontweight="bold")
        ax.text(output_x + 0.75, 4.8, "(Pos, Vel, Att)", ha="center", va="center", fontsize=7)
        ...
        # IMU to UKF
        ax.annotate(
            "", xy=(ukf_x, ukf_y + ukf_height - 0.5), xytext=(2.2, sensors_y + box_height / 2), arrowprops=arrow_props
        )

        # GNSS to UKF
        ax.annotate(
            "", xy=(ukf_x, ukf_y + ukf_height / 2), xytext=(2.2, sensors_y - 2 + box_height / 2), arrowprops=arrow_props
        )

        # Baro to UKF
        ax.annotate("", xy=(ukf_x, ukf_y + 0.5), xytext=(2.2, sensors_y - 4 + box_height / 2), arrowprops=arrow_props)

        if use_geo:
            # Gravity to UKF
            ax.annotate("", xy=(ukf_x, ukf_y + 0.2), xytext=(2.2, geo_y + box_height / 2), arrowprops=arrow_props)

            # Magnetic to UKF
            ax.annotate("", xy=(ukf_x, ukf_y), xytext=(2.2, geo_y - 1.5 + box_height / 2), arrowprops=arrow_props)

        # UKF to Output
        ax.annotate("", xy=(output_x, 5.25), xytext=(ukf_x + ukf_width, ukf_y + ukf_height / 2), arrowprops=arrow_props)
...
def create_summary_statistics():
    ...
    trajectories_no_outlier = {k: v for k, v in trajectories.items() if k != "2023-08-09_163741"}
    ...
    ax1.bar(
        x - width, baseline, width, label="GNSS-Degraded Baseline", color=TEMPLE_GRAY, edgecolor="black", linewidth=0.5
    )
    ax1.bar(x, gravity, width, label="Gravity-Aided", color=TEMPLE_CHERRY, edgecolor="black", linewidth=0.5)
    ax1.bar(x + width, magnetic, width, label="Magnetic-Aided", color="#D4AF37", edgecolor="black", linewidth=0.5)

    ax1.set_ylabel("Median Position Error (m)", fontsize=12, fontweight="bold")
    ax1.set_title(
        "Navigation Performance Across All Trajectories (Including Outlier)",
        fontsize=14,
        fontweight="bold",
        color=TEMPLE_CHERRY,
    )
    ...
    ax1.text(outlier_idx, 8500, "Outlier", ha="center", fontsize=9, color="red", fontweight="bold")
    ...
    ax2.bar(
        ...
        baseline_no,
        width,
        label="GNSS-Degraded Baseline",
        color=TEMPLE_GRAY,
        edgecolor="black",
        linewidth=0.5,
    )
    ax2.bar(x2, gravity_no, width, label="Gravity-Aided", color=TEMPLE_CHERRY, edgecolor="black", linewidth=0.5)
    ax2.bar(x2 + width, magnetic_no, width, label="Magnetic-Aided", color="#D4AF37", edgecolor="black", linewidth=0.5)

    ax2.set_ylabel("Median Position Error (m)", fontsize=12, fontweight="bold")
    ax2.set_xlabel("Trajectory", fontsize=12, fontweight="bold")
    ax2.set_title("Navigation Performance (Outlier Excluded)", fontsize=14, fontweight="bold", color=TEMPLE_CHERRY)
    ax2.set_xticks(x2)
