...
\begin{frame}{Geophysical Navigation: A Passive Alternative}
    \begin{columns}
        \begin{column}{0.55\textwidth}
            ...
            \end{itemize}
            \vspace{0.5em}
            \textbf{Prior art:} demonstrated in aerospace/maritime with \emph{navigation-grade} sensors (RLG/FOG IMUs, dedicated gravimeters)
        \end{column}
        \begin{column}{0.42\textwidth}
...
\begin{frame}{Prior Work and This Contribution}
    \begin{columns}
        \begin{column}{0.52\textwidth}
            \textbf{Prior work (Brodovsky \& Dames, ION 2025):}
            \begin{itemize}
...
\begin{frame}{Example: Strong Performance}
    \begin{figure}
        ...
        \end{subfigure}
        \caption{Trajectory 2023-08-06: geophysically-aided RBPF (orange) vs.\ RBPF baseline (blue) and UKF baseline (green). All three modalities show substantial, consistent improvement.}
    \end{figure}
\end{frame}
...
\begin{frame}{Example: Combined Aiding Synergy}
    \begin{figure}
        ...
        \begin{subfigure}[b]{0.32\textwidth}
            ...
            \caption{Combined}
        \end{subfigure}
        \caption{Trajectory 2025-07-08: combined anomaly aiding yields substantial improvement over either single modality. Gravity and magnetic anomalies resolve ambiguities that neither can address alone.}
    \end{figure}
    \begin{exampleblock}{}
        \small
    ...
    \begin{columns}
        ...
        \begin{column}{0.42\textwidth}
            \begin{block}{Key Takeaway}
                The RBPF is a viable architecture for MEMS-grade geophysical navigation. It handles non-Gaussian posteriors from map-matching without the $O(N^{16})$ cost of a full particle filter---but the primary driver of accuracy remains the \textbf{information content of the anomaly measurement}, not the filter alone.
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Limitations \& Future Work}
    \begin{columns}
        \begin{column}{0.50\textwidth}
            ...
            \begin{itemize}
                \item Simple Gaussian measurement model---ignores:
                    \begin{itemize}
                        \item Directional dependence of scalar magnetometer
                        \item Altitude corrections, temporal variation
                        \item Platform magnetic signature
                    \end{itemize}
                \item Weighted-mean state estimate can lie between particle clusters in multimodal cases
                \item Performance is field-dependent with no online observability estimate
            \end{itemize}
        \end{column}
        \begin{column}{0.47\textwidth}
...
\begin{frame}{Summary}
    \begin{center}
        \Large
        \textbf{Geophysical anomaly navigation with MEMS-grade sensors\\
        using a Rao-Blackwellized Particle Filter is viable.}
    \end{center}
    \vspace{0.5em}
    \begin{columns}
        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{What we showed}
                \begin{itemize}
                    \item RBPF successfully fuses gravity \& magnetic anomalies with degraded GNSS and low-cost INS
                    \item Consistent improvement across 21 diverse trajectories
                    \item Combined aiding provides synergistic benefit
                \end{itemize}
            \end{exampleblock}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Open-source tools}
                \texttt{strapdown-rs}: Rust library for high-performance INS algorithms\\[0.3em]
                RBPF implementation, GNSS degradation simulation, geophysical map integration---all open source.
            \end{block}
        \end{column}
    \end{columns}
