...
impl Particle {
    pub fn new(nav_state: StrapdownState, other_states: Option<DVector<f64>>, weight: f64) -> Particle {
        let state_size = 9 + match &other_states { Some(states) => states.len(), None => 0 };
        Particle { nav_state, other_states, state_size, weight }
    }
}
impl From<(DVector<f64>, f64)> for Particle {
    fn from(tuple: (DVector<f64>, f64)) -> Self {
        let (state_vector, weight) = tuple;
        assert!(state_vector.len() >= 9, "State vector must be at least 9 elements long");
        ...
        let other_states = if state_vector.len() > 9 { Some(state_vector.rows(9, state_vector.len() - 9).clone_owned()) } else { None };
...
pub enum ParticleAveragingStrategy { WeightedAverage, UnweightedAverage, HighestWeight }
impl Default for ParticleAveragingStrategy { fn default() -> Self { ParticleAveragingStrategy::WeightedAverage } }

#[derive(Clone, Debug)]
pub enum ParticleResamplingStrategy { Naive, Systematic, Multinomial, Residual, Stratified, Adaptive }
impl Default for ParticleResamplingStrategy { fn default() -> Self { ParticleResamplingStrategy::Residual } }
...
impl ParticleResamplingStrategy {
    ...
    fn residual_resample(particles: Vec<Particle>) -> Vec<Particle> {
        ...
        if residual_particles > 0 {
            ...
            for _ in 0..residual_particles { positions.push(u); u += step; }
            let mut i = 0; let mut j = 0; let mut cumsum = residual[0];
            while j < residual_particles {
                while positions[j] > cumsum { i += 1; cumsum += residual[i]; }
                let mut new_particle = particles[i].clone();
                new_particle.weight = 1.0 / n as f64;
                new_particles.push(new_particle);
                j += 1;
...
impl Debug for ParticleFilter {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let effective_particles = 1.0 / self.particles.iter().map(|p| p.weight * p.weight).sum::<f64>();
        ...
        f.debug_struct("ParticleFilter")
            ...
            .field("mean_position", &format_args!("({:.6}°, {:.6}°, {:.2}m)", mean[0].to_degrees(), mean[1].to_degrees(), mean[2]))
            .field("mean_velocity", &format_args!("({:.3}, {:.3}, {:.3}) m/s", mean[3], mean[4], mean[5]))
            .field("mean_attitude", &format_args!("({:.3}, {:.3}, {:.3}) rad", mean[6], mean[7], mean[8]))
            .finish()
    }
}
impl ParticleFilter {
    pub fn new(particles: Vec<Particle>, process_noise_std: Option<DVector<f64>>, estimation_strategy: Option<ParticleAveragingStrategy>, resampling_method: Option<ParticleResamplingStrategy>) -> Self {
        ...
        let process_noise: DVector<f64> = match process_noise_std {
            ...
            None => {
                ...
                if state_size > 9 { noise.push(1e-7); noise.push(1e-7); noise.push(1e-7); if state_size > 12 { noise.push(1e-9); noise.push(1e-9); noise.push(1e-9); } }
                while noise.len() < state_size { noise.push(0.0); }
                DVector::from_vec(noise)
            }
        };
        ParticleFilter { particles, process_noise, averaging_strategy: estimation_strategy.unwrap_or(ParticleAveragingStrategy::WeightedAverage), resampling_strategy: resampling_method.unwrap_or(ParticleResamplingStrategy::Residual), state_size }
    }
    pub fn particles_to_matrix(&self) -> DMatrix<f64> {
        ...
        for particle in &self.particles {
            ...
            data.push(euler.1);
            data.push(euler.2);
            if let Some(ref other_states) = particle.other_states { for val in other_states.iter() { data.push(*val); } }
        }
        DMatrix::from_vec(state_size, n_particles, data)
    }
    pub fn set_weights(&mut self, weights: &[f64]) { assert_eq!(weights.len(), self.particles.len()); for (particle, &w) in self.particles.iter_mut().zip(weights.iter()) { particle.weight = w; } }
    pub fn normalize_weights(&mut self) {
        let sum: f64 = self.particles.iter().map(|p| p.weight).sum();
        if sum > 0.0 && sum.is_finite() { for particle in &mut self.particles { particle.weight /= sum; } } else { let uniform = 1.0 / self.particles.len() as f64; for particle in &mut self.particles { particle.weight = uniform; } }
    }
    pub fn resample(&mut self) { self.particles = self.resampling_strategy.resample(self.particles.clone()); }
    pub fn effective_sample_size(&self) -> f64 { let sum_of_squares: f64 = self.particles.iter().map(|p| p.weight * p.weight).sum(); if sum_of_squares > 0.0 { 1.0 / sum_of_squares } else { 0.0 } }
}
impl crate::kalman::NavigationFilter for ParticleFilter {
    fn predict(&mut self, imu_data: IMUData, dt: f64) {
        let mut rng = rand::rng();
        for particle in &mut self.particles {
            let accel_biases = if particle.other_states.is_some() && particle.other_states.as_ref().unwrap().len() >= 6 { DVector::from_vec(vec![particle.other_states.as_ref().unwrap()[0], particle.other_states.as_ref().unwrap()[1], particle.other_states.as_ref().unwrap()[2]]) } else { DVector::from_vec(vec![0.0,0.0,0.0]) };
            let gyro_biases = if particle.other_states.is_some() && particle.other_states.as_ref().unwrap().len() >= 6 { DVector::from_vec(vec![particle.other_states.as_ref().unwrap()[3], particle.other_states.as_ref().unwrap()[4], particle.other_states.as_ref().unwrap()[5]]) } else { DVector::from_vec(vec![0.0,0.0,0.0]) };
            ...
            particle.nav_state.attitude = Rotation3::from_euler_angles(roll + roll_noise, pitch + pitch_noise, yaw + yaw_noise);
            if let Some(ref mut other_states) = particle.other_states { for i in 0..other_states.len().min(6) { let noise = rand_distr::Normal::new(0.0, self.process_noise[9 + i]).unwrap().sample(&mut rng) * dt_sqrt; other_states[i] += noise; } }
        }
    }
    ...
    fn get_certainty(&self) -> DMatrix<f64> {
        match self.averaging_strategy {
            ParticleAveragingStrategy::WeightedAverage => { let (_mean, cov) = ParticleAveragingStrategy::weighted_average_state(self); cov }
            ParticleAveragingStrategy::UnweightedAverage => { let (_mean, cov) = ParticleAveragingStrategy::unweighted_average_state(self); cov }
            ParticleAveragingStrategy::HighestWeight => { let (_mean, cov) = ParticleAveragingStrategy::highest_weight_state(self); cov }
        }
    }
}
...
impl ParticleAveragingStrategy {
    fn weighted_average_state(pf: &ParticleFilter) -> (DVector<f64>, DMatrix<f64>) {
        ...
        for particle in &pf.particles {
            ...
            if let Some(ref other) = particle.other_states { for (i, val) in other.iter().enumerate() { if 9 + i < state_size { mean[9 + i] += particle.weight * val; } } }
        }
        let mut cov = DMatrix::<f64>::zeros(state_size, state_size);
        for particle in &pf.particles {
            let euler = particle.nav_state.attitude.euler_angles();
            let mut state_vec = vec![particle.nav_state.latitude, particle.nav_state.longitude, particle.nav_state.altitude, particle.nav_state.velocity_north, particle.nav_state.velocity_east, particle.nav_state.velocity_down, euler.0, euler.1, euler.2];
            if let Some(ref other) = particle.other_states { state_vec.extend(other.iter()); }
    ...
    fn unweighted_average_state(pf: &ParticleFilter) -> (DVector<f64>, DMatrix<f64>) {
        ...
        for particle in &pf.particles {
            ...
            mean[7] += euler.1 / n;
            mean[8] += euler.2 / n;
            if let Some(ref other) = particle.other_states { for (i, val) in other.iter().enumerate() { if 9 + i < state_size { mean[9 + i] += val / n; } } }
        }
        let mut cov = DMatrix::<f64>::zeros(state_size, state_size);
        for particle in &pf.particles {
            let euler = particle.nav_state.attitude.euler_angles();
    ...
    fn highest_weight_state(pf: &ParticleFilter) -> (DVector<f64>, DMatrix<f64>) {
        let best_particle = pf.particles.iter().max_by(|a, b| a.weight.partial_cmp(&b.weight).unwrap()).expect("Particle filter has no particles");
        let euler = best_particle.nav_state.attitude.euler_angles();
        let mut state_vec = vec![best_particle.nav_state.latitude, best_particle.nav_state.longitude, best_particle.nav_state.altitude, best_particle.nav_state.velocity_north, best_particle.nav_state.velocity_east, best_particle.nav_state.velocity_down, euler.0, euler.1, euler.2];
        if let Some(ref other_states) = best_particle.other_states { state_vec.extend(other_states.iter()); }
