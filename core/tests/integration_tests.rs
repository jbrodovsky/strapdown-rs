...
/// Test particle filter closed-loop on real data
///
/// This test runs a closed-loop particle filter with GNSS measurements on real data and verifies that:
/// 1. The filter completes without errors
/// 2. Position errors remain bounded
/// 3. The filter performs with similar accuracy to the UKF
#[test]
...
fn test_particle_filter_with_degraded_gnss() {
    ...
    // Print statistics
    println!("\n=== Particle Filter with Degraded GNSS (5s updates) Error Statistics ===");
    println!(
        "Horizontal Error: mean={:.2}m, max={:.2}m, rms={:.2}m",
        stats.mean_horizontal_error, stats.max_horizontal_error, stats.rms_horizontal_error
    );
...
/// Test that particle filter performs similarly to UKF
///
/// This test runs both UKF and particle filter on the same data and verifies that
/// the particle filter achieves similar performance to the UKF, demonstrating that
/// the particle filter implementation is working correctly.
#[test]
fn test_particle_filter_performance_comparable_to_ukf() {
    // Load test data
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let test_data_path = Path::new(manifest_dir).join("tests/test_data.csv");
    let records = load_test_data(&test_data_path);
    ...
    // Print comparison
    println!("\n=== UKF vs Particle Filter Performance Comparison ===");
    println!(
        "UKF RMS Horizontal Error: {:.2}m",
        ukf_stats.rms_horizontal_error
    );
    println!(
        "Particle Filter RMS Horizontal Error: {:.2}m",
    ...
    println!(
        "Particle Filter RMS Altitude Error: {:.2}m",
        pf_stats.rms_altitude_error
    );

    // Particle filter should have reasonable performance but will be worse than UKF
    // since it doesn't estimate IMU biases (9 states vs 15 states)
    // Allow for larger tolerance due to lack of bias estimation and stochastic nature
    assert!(
        pf_stats.rms_horizontal_error < ukf_stats.rms_horizontal_error * 3.0,
        "Particle filter RMS horizontal error should be within 3x of UKF. PF: {:.2}m, UKF: {:.2}m",
        pf_stats.rms_horizontal_error,
        ukf_stats.rms_horizontal_error
    );

    assert!(
        pf_stats.rms_altitude_error < ukf_stats.rms_altitude_error * 10.0,
        "Particle filter RMS altitude error should be within 10x of UKF. PF: {:.2}m, UKF: {:.2}m",
        pf_stats.rms_altitude_error,
        ukf_stats.rms_altitude_error
    );

    // Particle filter should meet reasonable thresholds given no bias estimation
    assert!(
        pf_stats.rms_horizontal_error < 100.0,
        "Particle filter should have RMS horizontal error < 100m, got {:.2}m",
        pf_stats.rms_horizontal_error
    );
    assert!(
        pf_stats.rms_altitude_error < 200.0,
        "Particle filter should have RMS altitude error < 200m, got {:.2}m",
        pf_stats.rms_altitude_error
    );
}
